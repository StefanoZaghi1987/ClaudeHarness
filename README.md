...
| If you want to… | Read |
|---|---|
| understand how work actually moves — the phases, the gates, the artifacts, and the six routes | **[WORKFLOW.md](WORKFLOW.md)** |
| pick a tool for a phase, or check whether two tools compose | [workflow/TOOLS.md](workflow/TOOLS.md) |
| choose between a skill, an agent, or both, for writing or reviewing a spec, a plan, or a diff | [workflow/WRITE-AND-REVIEW.md](workflow/WRITE-AND-REVIEW.md) |
| turn a closed `wayfinder` map into a set of specs | [workflow/MAP-TO-SPEC.md](workflow/MAP-TO-SPEC.md) |
| know what each reviewer subagent checks and which gate it runs | [agents/README.md](agents/README.md) |
| see where each skill comes from, and what was deliberately left out | [skills/README.md](skills/README.md) |
...
| **artifact** | What crosses the boundary between two phases: a file, at a known path, that the next phase reads. Anything not written into one is dropped at the boundary. |
| **route** | One worked path through all five phases — a tool per phase, chosen from the shape of the job. Advisory, never normative. |
...
Every gate is two reviews in sequence — a reviewer subagent with a fresh context window, then your approval — and a blocking finding sends the work back one phase instead of forward. Which subagent runs which gate, and what each one checks, is in **[WORKFLOW.md](WORKFLOW.md)** and [agents/README.md](agents/README.md).

Each phase can be implemented in more than one way, and which tool you pick is a separate, changeable decision. Brainstorming, for example, runs on `interview-me`, `idea-refine`, `grilling`, `brainstorming` or `wayfinder` depending on how clear the idea already is — five skills and no subagent, although the `architect` subagent can close the phase when the one remaining question is structural. **[WORKFLOW.md](WORKFLOW.md)** gives every phase its goal, its exit criterion and the artifact it leaves behind, plus six worked routes from a one-line change to a multi-session map. [workflow/TOOLS.md](workflow/TOOLS.md) catalogues every tool, and the four rules that decide whether two of them compose. [workflow/WRITE-AND-REVIEW.md](workflow/WRITE-AND-REVIEW.md) walks the choice between a skill, an agent, or both at each writing and reviewing moment.
...
```
├── WORKFLOW.md          # the phases, the gates, the artifacts, and six worked routes
├── workflow/            # TOOLS.md (tool catalogue + composition rules), MAP-TO-SPEC.md (one runbook), WRITE-AND-REVIEW.md (the write/review playbook)
...
├── SECURITY.md          # the trust model of the resync engine, and how to report an issue
...
All four refer to models by alias, never by version number. `architect` is the only generative one, and the only one on the top tier. `code-reviewer` is the only agent in this repository with `Bash`, so it can collect its own `git diff`; the other three are read-only.
...
| `skills-resync` | Re-syncs the thirty-nine vendored skills in `~/.claude/skills` against their upstream plugins or tracked repos, replaying protected local edits across each re-vendor (details below). |
...
These two **maintain** the setup. The skills that **do the work** — `interview-me`, `spec-driven-development`, `writing-plans`, `incremental-implementation` and thirty-five more — do not ship here. They come from six plugins and one tracked repo, and are vendored into `~/.claude/skills`, which is exactly why `skills-resync` exists.
...
**[skills/README.md](skills/README.md)** is the supply-chain document: it groups the fleet by source, explains the trade-off between installing a plugin, vendoring a skill and keeping a plugin idle, and records what was deliberately left out and why. What each skill *does* lives in [workflow/TOOLS.md](workflow/TOOLS.md) instead — one home per fact.
...
Replace `v1.4.0` with the [latest tag](https://github.com/StefanoZaghi1987/ClaudeHarness/releases). The zip contains `agents/`, `skills/`, `rules/` at the top level, so it expands straight into `~/.claude/` and overwrites files with the same names:

```bash
curl -fsSL -o /tmp/claude-harness.zip \
  https://github.com/StefanoZaghi1987/ClaudeHarness/releases/download/v1.4.0/claude-harness-v1.4.0.zip
unzip -o /tmp/claude-harness.zip -d ~/.claude/
```

```powershell
Invoke-WebRequest https://github.com/StefanoZaghi1987/ClaudeHarness/releases/download/v1.4.0/claude-harness-v1.4.0.zip -OutFile $env:TEMP\claude-harness.zip
