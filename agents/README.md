...
Which tool reviews which object — these gate agents, the review skills that feed them, and
your approval — is mapped in [Who reviews what](../WORKFLOW.md#who-reviews-what) in
...
`Bash` for exactly one reason — it runs `git diff` or `git diff --staged` and collects the
diff itself, so you do not have to paste one in.
